...
def prepare_datasets(train_file_path, valid_file_path, test_file_paths, split_ratio=0.4, seed=2):
    ...
    combined_pre = torch.cat([transformed_pretrain_train, targets_pretrain_train], dim=0)
    mean_pre = combined_pre.mean()
    std_pre = combined_pre.std()
    ...
    combined_fine = torch.cat([transformed_finetune_train, targets_finetune_train], dim=0)
    mean_fine = combined_fine.mean()
    std_fine = combined_fine.std()
    ...
    for transformed_test, targets_test in zip(transformed_finetune_test_list, targets_finetune_test_list):
        combined_test = torch.cat([transformed_test, targets_test], dim=0)
        mean_test = combined_test.mean()
        std_test = combined_test.std()
