...
parser.add_argument('--best_model_path_pretrain', type=str, default='best_model_pretrain_train1.pth', help='Path to save the best pre-trained model.')
parser.add_argument('--best_model_path_finetune', type=str, default='best_finetune_train1.pth', help='Path to save the best fine-tuned model.')
...
parser.add_argument('--output_dir', type=str, default='./logs_train1', help='Directory for saving outputs')
parser.add_argument('--pretrained_dir', type=str, default='./pretrained_train1', help='Directory for pretrained models')
...
# TODO: Change augmentations to 50 for modelnet
parser.add_argument('--num_augmentations_pretrain', type=int, default=50, help='Number of augmentations for pre-training.')
...
parser.add_argument('--num_augmentations_finetune', type=int, default=50, help='Number of augmentations for fine-tuning.')
...
# Provide path for modelnet10 dataset zip file
parser.add_argument('--dir_model_net', type=str, default='./Voxelized_modelnet10',
                    help='Base directory containing ModelNet10 subfolders (e.g. bathtub, bed, chair, etc.)')
...
def unzip_if_needed(zip_path, extract_dir):
    ...
    if not os.path.exists(extract_dir):
        ...
        print(f"[Unzip] Extracting from {zip_path} to {extract_dir} ...")
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            zip_ref.extractall(extract_dir)
    else:
        print(f"[Unzip] Directory {extract_dir} already exists. Skipping extraction.")


def collect_modelnet10_data_for_all(data_dir, data_key='instance'):
    data_by_class = {}
    for class_name in sorted(os.listdir(data_dir)):
        # Skip 'demo' 
        if class_name.lower() == "demo":
            continue
        class_path = os.path.join(data_dir, class_name, "32")
        ...
        train_mat_files = sorted(glob.glob(os.path.join(train_folder, "*.mat")))
        test_mat_files  = sorted(glob.glob(os.path.join(test_folder,  "*.mat")))
        if not train_mat_files or not test_mat_files:
            continue

        train_list = []
        for mat_path in train_mat_files:
            try:
                data_dict = scipy.io.loadmat(mat_path)
                if data_key not in data_dict:
                    continue
                vol_np = data_dict[data_key]
                if vol_np.ndim == 4 and vol_np.shape[-1] == 1:
                    vol_np = np.squeeze(vol_np, axis=-1)
                if vol_np.ndim != 3:
                    continue
                vol_t = torch.tensor(vol_np, dtype=torch.float32).unsqueeze(0)
                train_list.append(vol_t)
            except:
                continue

        test_list = []
        for mat_path in test_mat_files:
            try:
                data_dict = scipy.io.loadmat(mat_path)
                if data_key not in data_dict:
                    continue
...
def prepare_datasets(data_dir, split_ratio=0.4, seed=2):
    data_dict = collect_modelnet10_data_for_all(data_dir, data_key='instance')

    if not data_dict:
        raise RuntimeError("No classes found that have both train and test volumes.")
    all_train_vols = []
    class_names = sorted(list(data_dict.keys()))
    for c in class_names:
        all_train_vols.extend(data_dict[c]["train"]) 

    all_train_tensor = torch.stack(all_train_vols, dim=0)

    def split_tensor(tensor, ratio, seed):
        torch.manual_seed(seed)
        idx = torch.randperm(tensor.size(0))
        split_pt = int(ratio * tensor.size(0))
        first_idx = idx[:split_pt]
        second_idx= idx[split_pt:]
        return tensor[first_idx], tensor[second_idx]

    train_40, train_60 = split_tensor(all_train_tensor, split_ratio, seed)
    pretrain_train_subtomograms, pretrain_valid_subtomograms = split_tensor(train_40, split_ratio, seed)
    finetune_train_subtomograms, finetune_valid_subtomograms = split_tensor(train_60, split_ratio, seed)

    tpt_train,  tpt_label,  ppt_train  = create_self_supervised_pairs(
        pretrain_train_subtomograms,
        pretrain=True,
        num_augmentations=args.num_augmentations_pretrain,
        test=False,
        max_rotation_angle=args.rotation_range_pretrain,
        translation_range=args.translation_range_pretrain,
        shearing_range=args.shearing_range_pretrain
    )
    tpt_valid,  tptv_label, ppt_valid  = create_self_supervised_pairs(
        pretrain_valid_subtomograms,
        pretrain=True,
        num_augmentations=args.num_augmentations_pretrain,
        test=False,
        max_rotation_angle=args.rotation_range_pretrain,
    ...
    tft_train,  tft_label,  pft_train  = create_self_supervised_pairs(
        finetune_train_subtomograms,
        pretrain=False,
        num_augmentations=args.num_augmentations_finetune,
        test=False,
        max_rotation_angle=args.rotation_range_finetune,
        translation_range=args.translation_range_finetune,
        shearing_range=args.shearing_range_finetune
    )
    tft_valid,  tftv_label, pft_valid  = create_self_supervised_pairs(
        finetune_valid_subtomograms,
        pretrain=False,
        num_augmentations=args.num_augmentations_finetune,
        test=False,
        max_rotation_angle=args.rotation_range_finetune,
    ...
    test_class_names       = []
    for c in class_names:
        test_vols = data_dict[c]["test"]
        test_tensor = torch.stack(test_vols, dim=0)

        t_test, t_label, p_test = create_self_supervised_pairs(
            test_tensor,
            pretrain=False,
            num_augmentations=1,
            test=True,
            max_rotation_angle=args.rotation_range_test,
            translation_range=args.translation_range_test,
            shearing_range=args.shearing_range_finetune
        )
        test_datasets_finetune.append((t_test, t_label, p_test))
        test_class_names.append(c)

    trans_pre_mean = ppt_train.mean(dim=0, keepdim=True)
    trans_pre_std  = ppt_train.std(dim=0, keepdim=True)
    if dist.is_initialized():
        dist.all_reduce(trans_pre_mean, op=dist.ReduceOp.SUM)
        dist.all_reduce(trans_pre_std,  op=dist.ReduceOp.SUM)
        wsize = dist.get_world_size()
        trans_pre_mean /= wsize
        trans_pre_std  /= wsize

    trans_fine_mean = pft_train.mean(dim=0, keepdim=True)
    trans_fine_std  = pft_train.std(dim=0, keepdim=True)
    if dist.is_initialized():
        dist.all_reduce(trans_fine_mean, op=dist.ReduceOp.SUM)
        dist.all_reduce(trans_fine_std,  op=dist.ReduceOp.SUM)
        wsize = dist.get_world_size()
        trans_fine_mean /= wsize
        trans_fine_std  /= wsize

    ppt_train_norm = (ppt_train - trans_pre_mean) / trans_pre_std
    ppt_valid_norm = (ppt_valid - trans_pre_mean) / trans_pre_std
    pft_train_norm = (pft_train - trans_fine_mean)/ trans_fine_std
    pft_valid_norm = (pft_valid - trans_fine_mean)/ trans_fine_std

    test_param_list_norm = []
    for p_test in [x[2] for x in test_datasets_finetune]:
        p_norm = (p_test - trans_fine_mean) / trans_fine_std
        test_param_list_norm.append(p_norm)

    mean_pre = tpt_train.mean()
    std_pre  = tpt_train.std()
    if dist.is_initialized():
        mpt = torch.tensor([mean_pre], device=tpt_train.device)
        spt = torch.tensor([std_pre], device=tpt_train.device)
        dist.all_reduce(mpt, op=dist.ReduceOp.SUM)
        dist.all_reduce(spt, op=dist.ReduceOp.SUM)
        w = dist.get_world_size()
        mean_pre = mpt.item()/w
        std_pre  = spt.item()/w

    tpt_train = (tpt_train - mean_pre)/(std_pre+1e-8)
    tpt_label = (tpt_label - mean_pre)/(std_pre+1e-8)
    tpt_valid = (tpt_valid - mean_pre)/(std_pre+1e-8)
    tptv_label= (tptv_label- mean_pre)/(std_pre+1e-8)

    mean_fine = tft_train.mean()
    std_fine  = tft_train.std()
    if dist.is_initialized():
        mft = torch.tensor([mean_fine], device=tft_train.device)
        sft = torch.tensor([std_fine],  device=tft_train.device)
        dist.all_reduce(mft, op=dist.ReduceOp.SUM)
        dist.all_reduce(sft, op=dist.ReduceOp.SUM)
        w = dist.get_world_size()
        mean_fine= mft.item()/w
        std_fine = sft.item()/w

    tft_train = (tft_train - mean_fine)/(std_fine+1e-8)
    tft_label = (tft_label - mean_fine)/(std_fine+1e-8)
    tft_valid = (tft_valid - mean_fine)/(std_fine+1e-8)
    tftv_label= (tftv_label- mean_fine)/(std_fine+1e-8)

    final_test_datasets = []
    for (idx, (test_input, test_label, _)) in enumerate(test_datasets_finetune):
        local_mean = test_input.mean()
        local_std  = test_input.std()

        test_input_norm = (test_input - local_mean)/(local_std+1e-8)
        test_label_norm = (test_label - local_mean)/(local_std+1e-8)
        test_params_norm= test_param_list_norm[idx]

        ds = TensorDataset(test_input_norm, test_label_norm, test_params_norm)
        final_test_datasets.append(ds)

    train_dataset_pretrain = TensorDataset(tpt_train, tpt_label, ppt_train_norm)
    valid_dataset_pretrain = TensorDataset(tpt_valid, tptv_label, ppt_valid_norm)
    train_dataset_finetune = TensorDataset(tft_train, tft_label, pft_train_norm)
    valid_dataset_finetune = TensorDataset(tft_valid, tftv_label, pft_valid_norm)
...
def train_model(
    ...
    for batch_idx, (inputs, targets, params) in enumerate(train_loader):
        ...
        # if (batch_idx + 1) % 10 == 0:
        #     trans_pred = transformation_pred[:, 9:]
        #     trans_gt   = gt_transformation[:, 9:]
        #     if trans_pred.numel() > 0:
        #         trans_pred_min = trans_pred.min().item()
        #         trans_pred_max = trans_pred.max().item()
        #         trans_gt_min   = trans_gt.min().item()
        #         trans_gt_max   = trans_gt.max().item()

        #         rot_pred = transformation_pred[:, :9]
        #         rot_gt   = gt_transformation[:, :9]
        #         if rot_pred.numel() > 0:
        #             rot_pred_min = rot_pred.min().item()
        #             rot_pred_max = rot_pred.max().item()
        #             rot_gt_min   = rot_gt.min().item()
        #             rot_gt_max   = rot_gt.max().item()
...
def test_model(model, test_loader, device, trans_fine_mean, trans_fine_std, test_file_name, transform_type='r9'):
    ...
    try:
        ...
        print(f"Rank {dist.get_rank()}: test_model => {test_file_name} processed {local_size} samples")
    ...
    if dist.is_initialized():
        if predicted_trans:
            all_preds = torch.cat(predicted_trans, dim=0)
            all_gt    = torch.cat(ground_truth_trans, dim=0)
            size_tensor = torch.tensor([all_preds.shape[0]], device=device)
            ...
            if all_preds.shape[0] < max_size:
                ...
                pad_preds = torch.zeros((pad_size, ) + all_preds.shape[1:], dtype=all_preds.dtype, device=all_preds.device)
                pad_gt    = torch.zeros((pad_size, ) + all_gt.shape[1:], dtype=all_gt.dtype, device=all_gt.device)
                all_preds = torch.cat([all_preds, pad_preds], dim=0)
                all_gt    = torch.cat([all_gt,    pad_gt],    dim=0)
            gathered_preds = [torch.zeros_like(all_preds) for _ in range(dist.get_world_size())]
            gathered_gt    = [torch.zeros_like(all_gt)    for _ in range(dist.get_world_size())]
            dist.all_gather(gathered_preds, all_preds)
            dist.all_gather(gathered_gt,    all_gt)
            predicted_trans = gathered_preds
            ground_truth_trans = gathered_gt
        else:
            predicted_trans = []
            ground_truth_trans = []
...
def evaluate_model(predicted_trans, ground_truth_trans, image_size=32, test_file_name="Test", transform_type='r9'):
    ...
    try:
        ...
        print(f"Error concatenating evaluate tensors: {e}")
        return
    ...
    if (ground_truth_trans.size(1) != expected_dim or 
        predicted_trans.size(1)    != expected_dim):
        print(f"[ERROR] Mismatch in transformation dims: GT={ground_truth_trans.size(1)}, Pred={predicted_trans.size(1)}, expected={expected_dim}")
        return

    print(f"\nEvaluating alignment on '{test_file_name}' with {predicted_trans.shape[0]} samples...")
    ...
    try:
        ...
        print(f"Error in alignment_eval: {e}")
...
def main():
    ...
    # Unzipping 
    # ===========================
    dataset_path = args.data_dir_modelnet

    if os.path.isfile(dataset_path) and dataset_path.endswith('.zip'):
        unzipped_dir = dataset_path.replace('.zip', '_unzipped')
        if dist.is_initialized() and dist.get_rank() == 0:
            unzip_if_needed(dataset_path, unzipped_dir)
        dist.barrier()
        dataset_path = unzipped_dir
    elif os.path.isdir(dataset_path):
        zip_files = [f for f in os.listdir(dataset_path) if f.endswith('.zip')]
        if zip_files:
            for zip_file in zip_files:
                zip_path = os.path.join(dataset_path, zip_file)
                unzipped_dir = os.path.join(dataset_path, zip_file.replace('.zip', '_unzipped'))
                if dist.is_initialized() and dist.get_rank() == 0:
                    unzip_if_needed(zip_path, unzipped_dir)
                dist.barrier()
                if len(zip_files) == 1:
                    dataset_path = unzipped_dir
                else:
                    raise RuntimeError(f"Multiple zip files found in {dataset_path}. Please ensure only one zip file is present.")
        else:
            if dist.is_initialized() and dist.get_rank() == 0:
                print(f"No zip files found in {dataset_path}. Assuming data is already unzipped.")
            dist.barrier()
    else:
        raise RuntimeError(f"Invalid data_dir_modelnet: {dataset_path}. Must be a zip file or a directory.")
    candidate_subfolder = os.path.join(dataset_path, "Voxelized_modelnet10")
    if os.path.isdir(candidate_subfolder):
        dataset_path = candidate_subfolder
        if dist.get_rank() == 0:
            print(f"[Info] Adjusted dataset_path to nested subfolder: {dataset_path}")
    ...
    datasets_tuple, test_class_names = prepare_datasets(
        data_dir=dataset_path,
        split_ratio=0.4,
        seed=SEED
    )
    ...
    for idx, test_loader in enumerate(test_loaders):
        class_name = test_class_names[idx] 
        test_file_name = class_name  
