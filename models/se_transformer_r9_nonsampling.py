...
from .layers.self_attention_r9 import SelfAttentionModule


def r9_to_matrix(r9):
    if r9.size(-1) != 9:
        raise ValueError(f"Last dimension must be 9, got {r9.size(-1)}")
    
    if r9.dtype != torch.float32:
        r9 = r9.float()
        
    R = r9.view(*r9.shape[:-1], 3, 3)
    with torch.cuda.amp.autocast(enabled=False):
        U, _, V = torch.linalg.svd(R)
        det = torch.linalg.det(torch.matmul(U, V.transpose(-2, -1)))
    V_adj = V.clone()
    V_adj[det < 0, :, -1] *= -1
    R_valid = torch.matmul(U, V_adj.transpose(-2, -1))
    return R_valid
...
# =============================
# SETransformerR9NonSampling Module
# =============================
class SETransformerR9NonSampling(nn.Module):
