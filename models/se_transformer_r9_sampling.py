...
def r9_to_matrix_ste(r9):
    if r9.size(-1) != 9:
        raise ValueError(f"r9_to_matrix_ste expects last dimension=9, got {r9.size(-1)}")
    with torch.autocast(device_type='cuda', enabled=False):
...
###############################################################################
class SETransformerR9Sampling(nn.Module):
    def __init__(
        self,
        in_channels,
